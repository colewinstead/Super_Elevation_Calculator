...
export default function AdminEntitlementsClient({ initialGrants }: { initialGrants: GrantRecord[] }) {
  ...
  const refresh = useCallback(async () => {
    ...
    const payload = await response.json() as { grants?: GrantRecord[]; error?: string };
  ...
  return (
    <div className="admin-grid">
      <section className="admin-card">
        ...
        <p>The customer must sign in once before their email appears in the account database.</p>
        <form className="admin-form" onSubmit={(event) => submit("grant", event)}>
          ...
          <div className="admin-actions"><button className="marketing-button primary-action" disabled={busy} type="submit">Grant Pro</button><button className="marketing-button danger-action" disabled={busy || !email} type="button" onClick={() => submit("revoke")}>Revoke manual Pro</button></div>
        </form>
