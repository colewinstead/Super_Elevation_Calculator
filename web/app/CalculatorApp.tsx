      ...
      <div className="workspace">
        ...
        <section className="panel results-panel">
          ...
          {!result ? <div className="results-empty"><div className="road-crown"><i></i><i></i></div><h3>Ready for curve inputs</h3><p>Enter PC, speed, and radius to calculate transition stations and signed lane slopes.</p></div> : <>
            ...
            <div className="lookup"><h3>Engineering lookup</h3><div><input placeholder="Station" value={lookupStation} onChange={(e) => setLookupStation(e.target.value)} /><input placeholder="Super (2%, 2, or 0.02)" value={lookupSlope} onChange={(e) => setLookupSlope(e.target.value)} /><button onClick={performLookup}>Lookup</button></div>{lookupResult && <pre>{JSON.stringify(lookupResult, null, 2)}</pre>}</div>
          </>}
        </section>
      </div>
