...
The original readiness changes improved identification, diagnostics, compatibility, and build repeatability. Engine `1.1.2` corrects both MDOT lane slopes at PC/PT to interpolate 70% of the way from normal crown to full superelevation and retains the optional tangent-midpoint transition for consecutive opposite-direction MDOT curves; those changes require version-specific engineering review. The separately selectable `tdot-rd11-2026-04-30` profile remains unchanged. The TDOT profile transcribes the desirable RD11-LR-1/RD11-LR-2 tables and RD11-SE-1 rules with golden and boundary tests, but still requires independent engineer review before production reliance.
