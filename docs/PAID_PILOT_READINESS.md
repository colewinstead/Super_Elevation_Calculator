...
The original readiness changes improved identification, diagnostics, compatibility, and build repeatability. Engine `1.2.3` centralizes MDOT lane profiles, keeps runoff segments linear, applies the SE-3A `X1 = Lr(NC/e)` breakpoint to the inside-lane rotation, and corrects normal-crown event labeling. Explicit reverse-curve pairs delete only the tangent runout between selected eligible opposite-direction curves, require `Tmin = 0.7Lr(exit) + 0.7Lr(entry)`, and preserve each curve's standard `e/Lr` rate with lane-specific continuous handoffs or normal-crown holds. A valid normal-crown hold is no longer rejected merely because its reporting midpoint is outside the PT-to-PC tangent. Short or invalid pairs block without altering the independent results. These changes require version-specific engineering review. The separately selectable `tdot-rd11-2026-04-30` profile remains unchanged. The TDOT profile transcribes the desirable RD11-LR-1/RD11-LR-2 tables and RD11-SE-1 rules with golden and boundary tests, but still requires independent engineer review before production reliance. See [`MDOT_TRANSITION_MODEL.md`](MDOT_TRANSITION_MODEL.md) for the source record and model boundaries.
